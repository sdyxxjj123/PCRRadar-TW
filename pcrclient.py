...
try:
    ...
    input("读取配置文件失败，请确定目录下config.json无误！")
    exit
ver = pinfo["ver"]
print(f'台服焊接雷达 By:光的速度')
print(f'配置文件加载成功！')
print(f'设定的游戏版本号为{ver}，请确认是否与游戏版本一致，若不一致请在config中修改')
defaultHeaders = {
...
class pcrclient:
    ...
    async def login(self):
        ...
        resl = await self.callapi('/load/index', {
            'carrier': 'Android'
        })
